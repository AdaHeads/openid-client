...
package body Util.Beans.Objects is
   ...
   --  Find the data type to be used for an arithmetic operation between two
   --  objects.
   function Get_Arithmetic_Type (Left, Right : Object) return Data_Type;

   --  Find the data type to be used for a composition operation between two
   --  objects.
   ...
   WString_Type  : aliased constant Wide_String_Type :=
                     Wide_String_Type '(others => <>);
   Flt_Type      : aliased constant Float_Type := Float_Type '(others => <>);
   Duration_Type : aliased constant Duration_Type_Def :=
                     Duration_Type_Def '(others => <>);
   ...
   generic
      ...
      with function Float_Comparator
        (Left, Right : Long_Long_Float) return Boolean;
      ...
      case Obj.V.Of_Type is
         ...
         when TYPE_STRING =>
            if Obj.V.String_Proxy /= null then
               Util.Concurrent.Counters.Increment
                 (Obj.V.String_Proxy.Ref_Counter);
            end if;

         when TYPE_WIDE_STRING =>
            if Obj.V.Wide_Proxy /= null then
               Util.Concurrent.Counters.Increment
                 (Obj.V.Wide_Proxy.Ref_Counter);
            end if;
   ...
   begin
      ...
      if Right.V.Of_Type = TYPE_TIME then
         return Result : Object do
            ...
            Result.V := Object_Value
              '(Of_Type    => TYPE_TIME,
                Time_Value =>
                  Duration_Operation (Left.Type_Def.To_Duration (Left.V),
                    Right.Type_Def.To_Duration (Right.V)));
         end return;
      end if;
      declare
      ...
      begin
         case T is
         when TYPE_INTEGER =>
            return To_Object (Int_Operation
                              (Left.Type_Def.To_Long_Long (Left.V),
              Right.Type_Def.To_Long_Long (Right.V)));

         when TYPE_FLOAT =>
            return To_Object (Float_Operation
                              (Left.Type_Def.To_Long_Float (Left.V),
   ...
   begin
      return Object '(Controlled with
          V      => Object_Value '(Of_Type    => TYPE_TIME,
                                   Time_Value =>
                                     Value.Type_Def.To_Duration (Value.V)),
   ...
   begin
      return Object '(Controlled with
          V      => Object_Value '(Of_Type     => TYPE_FLOAT,
                                   Float_Value =>
                                     Value.Type_Def.To_Long_Float (Value.V)),
   ...
   begin
      return Object '(Controlled with
          V      => Object_Value '(Of_Type   => TYPE_INTEGER,
                                   Int_Value =>
                                     Value.Type_Def.To_Long_Long (Value.V)),
   ...
   begin
      if Value.V.Of_Type = TYPE_STRING or
        Value.V.Of_Type = TYPE_WIDE_STRING
      then
         return Value;
      else
   ...
   begin
      case Obj.V.Of_Type is
         when TYPE_STRING =>
            if Obj.V.String_Proxy /= null then
               Util.Concurrent.Counters.Decrement
                 (Obj.V.String_Proxy.Ref_Counter, Release);
               if Release then
         ...
         when TYPE_WIDE_STRING =>
            if Obj.V.Wide_Proxy /= null then
               Util.Concurrent.Counters.Decrement
                 (Obj.V.Wide_Proxy.Ref_Counter, Release);
               if Release then
         ...
         when TYPE_BEAN =>
            if Obj.V.Proxy /= null then
               Util.Concurrent.Counters.Decrement
                 (Obj.V.Proxy.Ref_Counter, Release);
               if Release then
   ...
   begin
      ...
      if not (Bean_Proxy (Proxy.all).Bean.all in
                Util.Beans.Basic.List_Bean'Class)
      then
         return False;
      end if;
      declare
         L : constant Util.Beans.Basic.List_Bean_Access
           := Beans.Basic.List_Bean'Class
             (Bean_Proxy (Proxy.all).Bean.all)'Unchecked_Access;
      begin
   ...
   function To_Bean (Value : in Object) return access
     Util.Beans.Basic.Readonly_Bean'Class is
   ...
   begin
      return
        Object '(Controlled
                 with V        => Object_Value '(Of_Type     => TYPE_FLOAT,
                                                 Float_Value =>
   ...
   function To_Unbounded_Wide_Wide_String
     (Value : Object) return Unbounded_Wide_Wide_String is
   begin
      case Value.V.Of_Type is
         when TYPE_WIDE_STRING =>
         ...
         when others =>
            return To_Unbounded_Wide_Wide_String
              (To_Wide_Wide_String (To_String (Value)));
      end case;
   ...
   begin
      return To_Wide_Wide_String
        (Object_Type'Class (Type_Def).To_String (Value));
