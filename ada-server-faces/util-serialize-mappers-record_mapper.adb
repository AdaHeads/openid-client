...
package body Util.Serialize.Mappers.Record_Mapper is
   ...
   overriding
   function Clone
     (Handler : in Mapper) return Util.Serialize.Mappers.Mapper_Access is
   ...
   begin
      ...
      declare
         DE : constant Element_Data_Access :=
                Element_Data'Class (D.all)'Access;
      begin
   ...
   function Find_Mapper
     (Controller : in Proxy_Mapper;
      Name       : in String;
      Attribute  : in Boolean := False) return Mappers.Mapper_Access is
      Result : constant Mappers.Mapper_Access :=
                 Controller.Mapper.Find_Mapper (Name, Attribute);
   begin
      if Result /= null then
         return Result;
      else
         return Util.Serialize.Mappers.Mapper
           (Controller).Find_Mapper (Name, Attribute);
      end if;
   end Find_Mapper;
      ...
      begin
         if Map.Mapping /= null then
            declare
               ...
               Val : constant Util.Beans.Objects.Object :=
                       Getter (Element, M.Index);
            begin
