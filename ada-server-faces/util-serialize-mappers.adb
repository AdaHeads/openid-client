...
package body Util.Serialize.Mappers is
   ...
   --  -----------------------
   --  Add a mapping to associate the given <b>Path</b> to the mapper defined
   --  in <b>Map</b>. The <b>Path</b> string describes the matching node using
   --  a simplified XPath notation.
   --  Example :
   ...
   procedure Finalize (Controller : in out Mapper) is
      procedure Free is new
        Ada.Unchecked_Deallocation (Mapper'Class, Mapper_Access);
      procedure Free is new
        Ada.Unchecked_Deallocation (Mapping'Class, Mapping_Access);
   ...
   procedure Set_Member
     (Handler   : in Mapper;
      ...
      Map : constant Mapper_Access :=
              Mapper'Class (Handler).Find_Mapper (Name, Attribute);
   begin
      if Map /= null
        and then Map.Mapping /= null
        and then Map.Mapper /= null
      then
